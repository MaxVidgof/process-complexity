...
parser.add_argument("-s", "--step", dest="step", help="Sliding window step", default=1)
...
end_idx = window_size
block_number = 0
if(end_idx >= len(sorted_log)):
	blocks = 1
else:
	blocks = int(1+Complexity.math.ceil((len(sorted_log)-window_size)/step))
# Iterate over all blocks of window_size traces
while(start_idx==0 or end_idx < len(sorted_log) + step):
	# Filter PM4Py log
...
if(args.save_csv):
	...
	df_merged.to_csv(base_filename+"_metrics_sliding_window_"+str(args.window_size)+"step"+str(args.step)+".csv", index=False)
